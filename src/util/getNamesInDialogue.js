 ...
 * @return {Object} An Object with the names of the characters initialized with an empty string value
 */

export default function getNamesInDialogue(editorData) {
  const inputDom = extractBr(convertEditorDataToDom(editorData));
  const lines = Array.from(inputDom.querySelectorAll('p'), p => p.innerText);
