
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">

<html>

<head>
  <meta name="keywords" content="enstars, ensemble stars, ensemble stars wiki">
  <meta name="description" content="A text formatter for more efficiently uploading event/gacha stories to the Ensemble Stars Wiki.">
  <title>Story Formatter</title>

  <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js" integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN" crossorigin="anonymous"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.9/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>

  <script src='spectrum.js'></script>
  <link rel='stylesheet' href='spectrum.css' />

  <script type="text/javascript" src="main.js"></script>

  <link rel="icon" href="favicon.ico" type="image/x-icon">
  <link href="https://fonts.googleapis.com/css?family=Jura:600" rel="stylesheet">
  <link href="styles.css" rel="stylesheet">

  <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">

</head>

<body>

  <?php include(“includes/header.html”);?>


  <script src="header.html"></script>

    <div id="contents">
      <!---TABS ON TOP OF INPUT!--->
      <div class="tab">
        <button class="tablink" onclick="openTab(this, 'inputArea')">Text</button>
        <button class="tablink" onclick="openTab(this, 'detailArea')" id="defaultOpen">Details</button>
        <button class="tablink" onclick="openTab(this, 'renderArea')">Renders</button>
        <button class="tablink" onclick="openTab(this, 'tlArea')">TL Notes</button>
      </div>

      <!---CONTENTS OF TABS!--->
      <form id="everything">
        <!---INPUT TAB!--->
        <textarea id="inputArea" class="tabcontent" name="input"></textarea>

        <!---DETAILS TAB!--->
        <div id="detailArea" class="tabcontent">
          <!--Note: Bootstrap grid system has 12 columns in each row
              but columns do not have to add up to 12-->

          <div class="form-group row">
            <label class="col-form-label col-sm-3" for="location">Story Location</label>
            <div class="col-sm-6">
              <input class="form-control" type="text" id="location"><br>
            </div>
          </div>

          <div class="form-group row">
            <label for="author" class="col-form-label col-sm-3">Story Writer</label>
            <div class="col-sm-6">
              <select class="form-control" id="author">
                <option value="akira">日日日 (Akira)</option>
                <option value="yuuki">結城由乃 (Yuuki Yoshino)</option>
              </select>
            </div>
          </div>

          <div class="form-group row">
            <label class="col-form-label col-sm-3">Translator</label>
            <div class="col-sm-6">
              <input class="form-control" type="text" name="translator">
            </div>
          </div>
          <div class="row">
            <div class="col-sm-3"></div>
            <div class="col-sm-6">
              <input type="radio" name="tlLocation" value="wiki" checked> Translator is wiki username
            </div>
          </div>
          <div class="row">
            <div class="col-sm-3"></div>
            <div class="col-sm-6">
              <input type="radio" name="tlLocation" value="external"> External link for credit needed
            </div>
          </div>
          <div class="row">
            <div class="col-sm-3"></div>
            <div class="col-sm-6">
              <input class="form-control" type="text" name="tlLink"><br>
            </div>
          </div>


          <div class="row">
            <label class="col-form-label col-sm-10">Label Colors</label>
          </div>

          <div class="row">
            <label class="col-form-label col-sm-3">Writer</label>
            <div class="col-sm-9">
              <input class="jscolor" name="writerCol" value="fff"><br>
            </div>
          </div>

          <div class="row">
            <label class="col-form-label col-sm-3">Location</label>
            <div class="col-sm-9">
              <input class="jscolor" name="locationCol" value="fff"><br>
            </div>
          </div>

          <div class="row">
            <label class="col-form-label col-sm-3">Bottom</label>
            <div class="col-sm-9">
              <input class="jscolor" name="bottomCol" value="fff"><br>
            </div>
          </div>

          <div class="row">
            <label class="col-form-label col-sm-3">Text</label>
            <div class="col-sm-9">
              <input class="jscolor" name="textCol" value="fff"><br>
            </div>
          </div>

        </div>

        <!---RENDERTAB!--->
        <div id="renderArea" class="tabcontent">
          <p>
            Please paste in the filename as written in the wiki (including extension)
            of the dialogue render you want to use for each character. <br>
            (Forms will appear when you paste dialogue into the Text tab)
          </p>
            <label for="linkToRenders">Find renders for:</label>
            <select id="linkToRenders"></select>
            <button id="renderLinkBtn" onclick="openLink()" type="button">GO</button>
            <p></p>
            <div id="renderForms"></div>
          </div>

          <!---TLNOTETAB!--->
          <textarea id="tlArea" class="tabcontent" name="tl"></textarea>
          <button type="button" onclick="convertText()" id="convert">CONVERT</button>

        </form>

        <textarea id="outputArea"></textarea>

        </div>

        <footer></footer>

        <script>
        $(".jscolor").spectrum({
          flat: false,
          showInput: true,
          className: "full-spectrum",
          showInitial: true,
          preferredFormat: "hex",
        });

        setup() // makes small edits to page display

        $('input[name=tlLocation]').change(showExLink); //toggles input for translator link
        $('#inputArea').change(updateRenders); //makes inputs for each character in dialogue to insert render link

        </script>


</body>
</html>
